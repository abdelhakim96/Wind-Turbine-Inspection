...
offboard_control::offboard_control(const ros::NodeHandle &nh, const ros::NodeHandle &nh_private) :
    ...
    state_sub_ = nh_.subscribe<mavros_msgs::State>("/mavros/state", 1, &offboard_control::stateCallback, this,
                                                   ros::TransportHints().tcpNoDelay());
    // Ros Services
    ...
    // Action timers:
    cmdloop_timer_ = nh_.createTimer(ros::Duration(0.01), &offboard_control::cmdloopCallback,
                                     this);  // Define timer for constant loop rate
    statusloop_timer_ = nh_.createTimer(ros::Duration(1), &offboard_control::statusloopCallback,
                                        this);  // Define timer for constant loop rate
    // PARAMETERS:
...
void offboard_control::stateCallback(const mavros_msgs::State::ConstPtr &msg) {
    current_state_ = *msg;
}

void offboard_control::targetCallback(const geometry_msgs::TwistStamped &msg) {

}

void offboard_control::poseCallback_local(const geometry_msgs::PoseStamped &msg) {
...
void offboard_control::poseCallback_global(const nav_msgs::Odometry::ConstPtr &msg) const {
//    if (!received_home_pose) {
//        received_home_pose = true;
...
void offboard_control::cmdloopCallback(const ros::TimerEvent &event) {
    // State machine
    switch (node_state) {
...
void offboard_control::statusloopCallback(const ros::TimerEvent &event) {
...
bool offboard_control::init_drone() {
...
void offboard_control::print_state(const string &text) {
    if (last_text_ != text) {
...
bool offboard_control::check_waypoint_reached(float pos_tolerance, float heading_tolerance_deg) {
    target_pose_.header.stamp = ros::Time::now();
    local_pos_pub_.publish(target_pose_);
