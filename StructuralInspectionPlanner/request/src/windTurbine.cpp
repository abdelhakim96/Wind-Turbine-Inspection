...
int main(int argc, char **argv) {
    ros::init(argc, argv, "windTurbine");
    ROS_INFO("windTurbine is alive");
    if (argc != 1) {
        ROS_INFO("usage: plan");
        return 1;
    }
    ...
    srv.request.spaceSize.push_back(100);
    srv.request.spaceSize.push_back(300);
    ...
    srv.request.incidenceAngle = 75 * M_PI / 180; // RAD = deg * M_PI/180
    srv.request.minDist = 10.0;
    srv.request.maxDist = 13.0;
    srv.request.numIterations = 150;

    /* read STL file and publish to rviz */
    ROS_INFO("Reading stl");
    std::vector <nav_msgs::Path> *mesh = readSTLfile(ros::package::getPath("request") + "/meshes/t_with_side_flipped.stl");
    ...
    if (client.call(srv)) {
        ...
        for (std::vector<geometry_msgs::PoseStamped>::iterator it = srv.response.inspectionPath.poses.begin();
             it != srv.response.inspectionPath.poses.end(); it++) {
            tf::Pose pose;
            tf::poseMsgToTF(it->pose, pose);
            double yaw_angle = tf::getYaw(pose.getRotation());
            pathPublication << it->pose.position.x << ", " << it->pose.position.y << ", " << it->pose.position.z
                            << ", 0, 0, " << yaw_angle << ";\n";
        }
        pathPublication << "];\n";
        pathPublication << "inspectionCost = " << srv.response.cost << ";\n";
        pathPublication << "numObstacles = "
                        << std::min(srv.request.obstacles.size(), srv.request.obstaclesPoses.size()) << ";\n";
        for (int i = 0; i < std::min(srv.request.obstacles.size(), srv.request.obstaclesPoses.size()); i++) {
            pathPublication << "obstacle{" << i + 1 << "} = [" << srv.request.obstacles[i].dimensions[0] << ", "
                            << srv.request.obstacles[i].dimensions[1] << ", " << srv.request.obstacles[i].dimensions[2]
                            << ";\n";
            pathPublication << srv.request.obstaclesPoses[i].position.x << ", "
                            << srv.request.obstaclesPoses[i].position.y << ", "
                            << srv.request.obstaclesPoses[i].position.z << "];\n";
        }
        pathPublication << "meshX = [";
        for (std::vector<nav_msgs::Path>::iterator it = mesh->begin(); it != mesh->end() && ros::ok(); it++) {
            pathPublication << it->poses[0].pose.position.x << ", " << it->poses[1].pose.position.x << ", "
                            << it->poses[2].pose.position.x << ";\n";
        }
        pathPublication << "];\nmeshY = [";
        for (std::vector<nav_msgs::Path>::iterator it = mesh->begin(); it != mesh->end() && ros::ok(); it++) {
            pathPublication << it->poses[0].pose.position.y << ", " << it->poses[1].pose.position.y << ", "
                            << it->poses[2].pose.position.y << ";\n";
        }
        pathPublication << "];\nmeshZ = [";
        for (std::vector<nav_msgs::Path>::iterator it = mesh->begin(); it != mesh->end() && ros::ok(); it++) {
            pathPublication << it->poses[0].pose.position.z << ", " << it->poses[1].pose.position.z << ", "
                            << it->poses[2].pose.position.z << ";\n";
        }
        pathPublication << "];\n";
    } else {
